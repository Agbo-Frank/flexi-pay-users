...
export function LandingSlide(){
    ...
    return(
        <div className="w-full relative group">
            ...
            <ul className="absolute bottom-0 -translate-y-6 left-2/4 flex space-x-2">
                {
                    imgs.map((dot, dotIdx) => (
            ...
            <div 
                className={`absolute top-2/4 w-8 h-8 ${index === 0 && 'hidden'}  bg-black/50 rounded-full place-items-center cursor-pointer group-hover:grid hidden ml-3`}
                onClick={() => {
                    if(index > 0){
            ...
            <div className={`absolute top-2/4 right-0 rounded-full ${index === imgs.length - 1 && 'hidden'} bg-black/50 w-8 h-8 place-items-center cursor-pointer group-hover:grid hidden mr-3`}
            onClick={() => {
...
export function CategorySlide(){
    ...
    return(
        <div className="w-full relative group">
            ...
            <div 
                className={`absolute top-2/4 w-8 h-8 ${index === 0 && 'hidden'}  bg-black/50 rounded-full place-items-center cursor-pointer grid ml-2`}
                onClick={() => {
                    if(index > 0){
                        setIndex(state => state - 1)
                    }
            ...
            <div 
                className={`absolute top-2/4 right-0 rounded-full ${index === categories.length - 1 && 'hidden'} bg-black/50 w-8 h-8 place-items-center cursor-pointer grid mr-2`}
                onClick={() => {
